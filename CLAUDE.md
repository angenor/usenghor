...
| `usePeiPage()` | Cadre commun des rubriques PEI `/entrepreneuriat/{alumni,partenaires,ressources,actualites,statut-etudiant-entrepreneur}` (rubrique `see`) : éditorial, service DDE, hero, fil d'Ariane, `applySeo({ type? })` (JSON-LD `CollectionPage` par défaut, `WebPage` pour la page SEE) — `await` en tête de page, `applySeo()` en dernier |
