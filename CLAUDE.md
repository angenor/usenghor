...
| `EntrepreneurshipHomeHero` / `JourneyRing` / `StepFeature` / `PhaseCard` | Refonte visuelle de l'accueil PEI et de « Nos activités » : hero plein écran : l'accent passe d'un mot du slogan à l'autre en boucle (`hero.slogan`, mots terminés par un point), 3 images synchronisées (fondu, zoom lent, voile en dégradé côté texte ; tous les mots allumés avec `prefers-reduced-motion`), anneau SVG des dispositifs (segments = liens `#phase-*`), étape en grand format éditorial, cartes « trois phases » en escalier. **Couleur fixe par phase** (`PEI_PHASE_TONES` / `peiStepTone(phase)` dans `utils/pei-presentation.ts` : OSER orange, SEE violet, MTI rouge, Senghor'Innov vert, FSE turquoise, écosystème bleu) — `pei_programs.color` n'est plus lu côté public ni éditable (pastille en lecture seule dans `ProgramForm`). Variantes additives : `EventList variant="agenda"`, `NewsCard variant="featured"\|"compact"`, `PartnerFamilies variant="columns"` + prop `descriptions`, `QuoteBlock impactStats` |
| `usePeiPreview()` / `usePeiPreviewImages()` / `EntrepreneurshipPreviewBanner` | **Mini-site PEI en validation visuelle** (2026-09) : `PEI_PREVIEW_DEFAULT = true` → aperçu actif par défaut sur accueil, « Nos activités » et alumni (`?apercu=0` = état réel) ; les blocs vides sont complétés par des exemples (`bank/mock-data/pei-home-preview.ts`, `pei-alumni-preview.ts`), images = couvertures d'actualités / événements publiés lues à l'exécution (`image(slot)`), bandeau + `robots noindex`. `PEI_PUBLICLY_LISTED = false` → pôle masqué de l'organigramme, des pages secteur et fiches service (`usePublicOrganizationApi`), du menu, du pied de page et du sitemap (`isUnlistedPeiPath` / `isUnlistedPeiService`). `nuxt.config.ts` : `PEI_NOINDEX_PATHS` (en-tête `X-Robots-Tag` + exclusion du sitemap). **À l'ouverture : basculer les trois.** |
| `EntrepreneurshipAlumniHero` / `CohortTimeline` / `LaureateSpotlight` | Refonte de `/entrepreneuriat/alumni` : hero avec mosaïque de portraits, frise des promotions (ancres), portrait à la une ; une cohorte s'affiche même sans portrait ; utilitaires `utils/pei-alumni.ts` ; variantes additives `PillTabs variant="segmented"` (+ `count`), `StatsPanel variant="cards"`, `CtaBanner variant="rose"`, `StepFeature fallbackImage`, `QuoteBlock portraitPlaceholder` (portrait réel de l'auteur cherché dans l'équipe publique de la DDE) |
...
## Recent Changes
- pei-validation-visuelle: mini-site PEI en phase de validation — aperçu par défaut avec exemples (images de production), tous les blocs photo des maquettes remplis, écosystème et hero d'accueil conformes à la maquette, refonte de `/entrepreneuriat/alumni` ; accès publics masqués (`PEI_PUBLICLY_LISTED`) et noindex (`PEI_NOINDEX_PATHS`) ; incident : `/a-propos/organisation` en 500 car le code 026 avait été déployé sans la migration 050 (appliquée le 2026-09-24) — vérifier les migrations en prod avant chaque déploiement
