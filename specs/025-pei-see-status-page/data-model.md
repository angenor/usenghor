# Data Model — 025 Page SEE

Aucune table, colonne, énumération ni index nouveau. La feature **lit** trois sources existantes et **seed** des lignes par la migration 049 (soumise à accord, § 4).

## 1. Entités lues

### 1.1 Appel SEE — `application_calls` (+ sous-tables), lecture publique par slug

| Champ utilisé | Usage sur la page |
|---|---|
| `slug` | cible interne `/candidatures/postuler/{slug}` |
| `status` (`upcoming` / `ongoing` / `closed`) | état dérivé (§ 2.1) |
| `publication_status` | non publié → 404 serveur → état `absent` |
| `opening_date` (date) | année du badge ; « Ouverture le … » (à venir) |
| `deadline` (datetime tz) | état ; année (repli) ; repère « date limite » |
| `external_form_url` | cible externe (URL HTTP(S) valide seulement) |
| `use_internal_form` | cible interne autorisée |
| `schedule[]` : `step`, `start_date`, `end_date`, `description` (+ `_en`, `_ar`), `display_order` | frise de l'agenda |
| `eligibility_criteria[]` : `criterion` (+ `_en`, `_ar`), `is_mandatory`, `display_order` | carte « Conditions » (prioritaire) |
| `required_documents[]` : `document_name`, `description` (+ `_en`, `_ar`), `is_mandatory`, `display_order` | carte « Mon dossier » (prioritaire) |

Désignation : `editorial_contents['entrepreneurship.see.call_slug']` (045). Type attendu `training`, non bloquant.

### 1.2 FAQ SEE — `faq_categories` / `faq_entries`

- Catégories : `code` préfixé `see-` (réservé par convention, non modifiable en backoffice), `label_{fr,en,ar}`, `is_active`, `display_order`.
- Entrées : `slug` (ancre), `question_{fr,en,ar}`, `answer_{fr,en,ar}_html`, `is_published`, `display_order`, `published_at`.
- Lecture : `GET /api/public/faq?category_prefix=see-` ([contracts/public-api.md](contracts/public-api.md)). Repli FR côté serveur (questions, réponses, libellés).

### 1.3 Contenu éditorial — `editorial_contents` (catégorie `values`)

65 clés `entrepreneurship.see.*` nouvelles + 4 existantes (045) + réutilisation de `entrepreneurship.contact.email` et `entrepreneurship.dde_service_id`. Liste : [contracts/editorial-keys.md](contracts/editorial-keys.md).

## 2. Règles de présentation (utilitaires purs, `app/utils/pei-presentation.ts`)

### 2.1 `seeCallState(call, now) → 'open' | 'upcoming' | 'closed' | 'absent'`

| Entrée | Résultat |
|---|---|
| `call` nul (clé vide, 404, erreur réseau) | `absent` |
| `status = 'ongoing'` et (`deadline` nulle ou `deadline > now`) | `open` |
| `status = 'upcoming'` | `upcoming` (même si `opening_date` passée) |
| `status = 'closed'`, ou `ongoing` avec `deadline ≤ now` | `closed` |

### 2.2 `seeApplyTarget(call, state, email, subject) → { kind, href?, to? } | null`

| Condition | Cible |
|---|---|
| `state = 'open'` et `isHttpUrl(external_form_url)` | `external` → `href` (nouvel onglet, `noopener noreferrer`) |
| `state = 'open'` et `use_internal_form` | `internal` → `to = /candidatures/postuler/{slug}` (localisé) |
| sinon, `email` renseigné | `contact` → `href = mailto:{email}?subject={subject encodé}` |
| sinon | `null` (aucun bouton) |

### 2.3 `seeCallYear(call) → number | null`

Année de `opening_date`, sinon de `deadline`, sinon `null`.

### 2.4 `seeAgendaSteps(schedule, deadline, timeZone = 'Africa/Cairo') → AgendaStep[]`

`AgendaStep = { id, label, description, start, end, isDeadline, synthetic }`. Tri `display_order` ; `isDeadline` si le jour calendaire (fuseau donné) de `end ?? start` = jour de `deadline` (première correspondance seulement) ; aucune correspondance et `deadline` présente → ligne synthétique (`label = t('pei.see.deadline')`, date + heure) insérée avant la première étape dont `start` est postérieur, sinon en fin.

### 2.5 Listes éditoriales à emplacements

`seeSlots(text, prefix, count) → string[]` : valeurs non vides de `entrepreneurship.see.{prefix}.{1..count}`, dans l'ordre. Leviers : `{ icon, title, text }` gardés si `title` non vide ; `icon` vide ou ne commençant pas par `fa-` → `fa-solid fa-circle-check`.

### 2.6 Masquage

| Bloc | Masqué si |
|---|---|
| Intro (colonne gauche) | titre, accroche et paragraphe vides |
| Encadré « Le SEE, c'est quoi ? » | libellé et texte vides |
| Carte SEE 1 / SEE 2 | titre vide |
| Section « Pourquoi postuler ? » | aucun levier |
| Carte Conditions / Jury / Dossier | liste résultante vide |
| Panneau agenda | jamais (porte l'état de l'appel) |
| Section FAQ + JSON-LD `FAQPage` | aucune entrée publiée dans les catégories `see-*` actives |
| Groupe FAQ | catégorie sans entrée publiée |
| Ligne PÉPITE France | `see.pepite.url` non HTTP(S) ou libellé vide |
| Contact e-mail | `contact.email` vide |
| CTA final | titre vide |

## 3. États de l'agenda (vue)

```text
            clé vide / 404 / erreur ─────────────► absent  : message « appel clos » + contact ; pas de frise ; badge hero sans année
appel lu ─┬─ status=upcoming ─────────────────────► upcoming: frise + « Ouverture le … » + contact ; badge « Appel à venir »
          ├─ status=ongoing ∧ deadline future ────► open    : frise + bouton Postuler (externe | interne | contact) + note « directeur en copie »
          └─ status=closed ∨ deadline passée ─────► closed  : frise + message « appel clos » + contact ; badge « Appel clos »
```

Transitions pilotées par le backoffice des appels (statut saisi) et par le temps (date limite) ; aucune écriture par la page.

## 4. Migration 049 — SQL à valider (porte « accord → SQL → code »)

Fichiers : `usenghor_backend/documentation/modele_de_données/migrations/049_pei_see_page.sql` et `049_pei_see_page_rollback.sql`. Aucune modification de `services/*.sql` (pas de structure).

Effets attendus : +65 lignes `editorial_contents` (73 → 138 lignes `entrepreneurship.%` en local), +4 `faq_categories`, +8 `faq_entries` (2 publiées, 6 brouillons). Second passage : 0 insertion.
...
Points à confirmer avec l'accord :

1. **Adresse PÉPITE France** : seedée vide (lien masqué) faute d'adresse vérifiée ; fournir l'URL pour la seeder.
2. **Texte « appel clos »** (`see.closed.*`) : rédigé pour la feature (absent de la maquette).
3. **Libellés EN/AR des catégories** : traduction proposée ci-dessus.
4. **Ordre sur `/faq`** : `display_order` 10 à 13 (après « Général » et « Admissions », à 0).
5. **Rollback** : les 8 entrées seedées sont supprimées même si l'équipe les a complétées entre-temps.

## 5. Configuration éditoriale (frontend)

- `editorial-pages-config.ts`, section `entrepreneurship-see` : description « Textes de la page « Entreprendre et étudier », appel SEE en cours et encart « devenir mentor » » ; `editorialKeys` et `fields` + 65 entrées (types : `text` ; `textarea` pour `hero.subtitle`, `intro.lead`, `intro.body`, `what.text`, `tracks.*.text`, `levers.*.text`, `apply.intro`, `closed.text`, `cta.text` ; `image` pour `hero.image`), `defaultValue` = valeur seedée.
- `types/api/editorial.ts` : +65 littéraux `ValueSectionKey`.
- Décompte de contrôle : 73 + 65 = **138** clés `entrepreneurship.*` déclarées.

## 6. Backend (aucune structure)

- `FaqService.get_public_tree(category_prefix: str | None = None)` ; router `GET /api/public/faq` + `Query(None, pattern=r'^[a-z0-9_-]{1,60}$')`.
- `FaqService.autofill_entry_translations(entry)` (public, délègue à l'existant).
- `EntrepreneurshipService.translate_missing` : étape `faq_see` (entrées publiées, catégories `code LIKE 'see-%'` échappé, un champ EN/AR vide) ; `PeiTranslateMissingResponse.faq_see: int = 0`.
